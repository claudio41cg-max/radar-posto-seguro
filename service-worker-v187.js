/* Radar Seguro RJ PRO v187 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v187';
const CORE=[
  './radar-v187.html','./index.html','./manifest-v187.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v187.js',
  ...
  './route-style-v185.js','./route-cancel-cleanup-v186.js','./routing-integrity-v187.js','./navigation-recovery-v187.js','./route-alternatives-v187.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v187.html'):Response.error());}}
