...
window.addEventListener('pagehide',()=>{backgrounded=true;manualStop=true;cleanup(true)});
window.addEventListener('pageshow',()=>{if(backgrounded){backgrounded=false;manualStop=false;stage='idle';ws=null;setupReady=false;running=false;starting=false;micSending=false;introAudioReceived=false;setButtons(false);setStatus('',false);showDetail('')}});
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'){backgrounded=true;manualStop=true;cleanup(true)}else if(backgrounded){backgrounded=false;manualStop=false;stage='idle';ws=null;setupReady=false;running=false;starting=false;micSending=false;introAudioReceived=false;setButtons(false);setStatus('',false);showDetail('')}});
ensureUI();setButtons(false);window.RadarGeminiLiveV205={start,stop,toggle,get state(){return{running,starting,setupReady,micSending,introAudioReceived,stage,model:MODEL,wsState:ws?.readyState??-1}}};
})();