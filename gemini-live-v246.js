...
let liveConnectedAt=0,liveCloseDiag=null;
function renderLiveCloseDiag(){
  let el=document.getElementById('geminiLiveCloseDiagV246');
  if(!el){el=document.createElement('pre');el.id='geminiLiveCloseDiagV246';Object.assign(el.style,{position:'fixed',left:'18px',right:'18px',bottom:'18px',zIndex:'2147483647',margin:'0',padding:'12px',borderRadius:'14px',background:'rgba(0,0,0,.88)',color:'#fff',font:'700 14px/1.35 monospace',whiteSpace:'pre-wrap',pointerEvents:'none'});document.body.appendChild(el)}
  const d=liveCloseDiag;
  el.textContent=d?['DIAGNÓSTICO LIVE V246','LIVE: DESCONECTADO','TEMPO CONECTADO: '+(d.connectedForMs/1000).toFixed(1)+' s','FECHOU NA ETAPA: '+d.closedAt,'CÓDIGO WEBSOCKET: '+(d.code||'sem código'),'MOTIVO: '+(d.reason||'sem motivo'),'MANUAL: '+(d.manual?'SIM':'NÃO'),'MICROFONE ANTES DE FECHAR: '+(d.micActive?'ATIVO':'INATIVO'),'HORÁRIO: '+d.at].join('\n'):'';
}
...
async function executeRadarCommand(command){
  ...
  try{
    ...
    const result=await va.routeTo(destination,{fromGemini:true});
    const ok=Boolean(result?.ok);
    toolDiagV238.route=ok?'OK':'NÃO';
    toolDiagV238.navigation=result?.navigationActive?'OK':'NÃO';
    toolDiagV238.error=ok?'—':safeText(result?.error||'Rota não criada',180);
    stickyDiagV238();
    return result||{ok:false,error:'Sem resposta do fluxo original',destination};
  }catch(e){
