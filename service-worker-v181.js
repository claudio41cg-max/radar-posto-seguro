/* Radar Seguro RJ PRO v181 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v181';
const CORE=[
  './radar-v181.html',
  './index.html',
  './manifest-v181.json',
  ...
  './cesarao-antares-rollas-territory-v181.js',
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v181.html'):Response.error());}}
