/* Radar Seguro RJ PRO v222 — FOLLOW com ponto focal inferior correto.
   Regra: a coordenada do veiculo permanece como center geografico; padding superior maior
   desloca o centro aparente para baixo da tela, como following-frame de navegacao.
   Nao desloca o veiculo para frente na rota. */
(()=>{'use strict';
if(window.__radarNavigationCameraV222)return;window.__radarNavigationCameraV222=true;
...
function frame(ts){raf=requestAnimationFrame(frame);const a=app();if(!a?.navActive||!a?.map||!point(target)||Date.now()<manualUntil)return;if(ts-last<32)return;last=ts;const s=Math.max(0,+a.currentSpeed||0),k=s>=80?.32:s>=45?.27:s>=15?.23:.20;shown=[shown[0]+(target[0]-shown[0])*k,shown[1]+(target[1]-shown[1])*k];if(Number.isFinite(targetBearing))shownBearing=Number.isFinite(shownBearing)?blend(shownBearing,targetBearing,.15):targetBearing;const cfg=profile(s);
 ...
 /* IMPORTANTE: top > bottom. No MapLibre o padding muda o centro aparente.
    Isso coloca a seta aproximadamente em 62% da altura util, abaixo do centro,
    deixando a maior parte da tela para a estrada a frente. */
 const top=Math.round(clamp(h*.31,155,h*.36)),bottom=Math.round(clamp(h*.075,42,72));
...
function install(){const a=app();if(!a?.map)return false;if(installed)return true;installed=true;window.__RADAR_SINGLE_CAMERA_OWNER='v222';
...
window.RadarNavigationCameraV222={version:'222',refresh:()=>{manualUntil=0;capture(app())}};
})();