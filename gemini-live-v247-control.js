...
const SYSTEM_TEXT='Você é Radar, o copiloto de voz do Radar Seguro RJ Pro. Responda em português do Brasil, de forma curta, natural e direta. O usuário pode estar dirigindo. Use as ferramentas do aplicativo quando pedirem localização, busca de lugar, rota, casa ou trabalho. Nunca invente localização, rota, trânsito, ocorrências ou fatos atuais. Antes de dizer que uma rota foi iniciada, confirme pelo resultado da ferramenta.';
...
async function handleToolCall(tc){const calls=tc?.functionCalls||tc?.function_calls||[];if(!calls.length)return;const responses=[];for(const call of calls){const name=call?.name||'',id=call?.id||call?.callId;try{const result=await window.RadarAiToolsV224?.execute?.(name,call?.args||{});responses.push({id,name,response:{result:result??null}})}catch(e){responses.push({id,name,response:{error:String(e?.message||e)}})}}if(responses.length)send({toolResponse:{functionResponses:responses}})}
function handleServerMessage(m){
  ...
  if(m?.goAway){setStatus('Gemini Live • servidor vai reconectar');return} if(m?.toolCall){handleToolCall(m.toolCall).catch(e=>console.warn('Gemini Live tool',e));return}
