/* Radar Seguro RJ PRO v188 — linha principal um pouco mais robusta, sem alterar a geometria da rota. */
(()=>{'use strict';if(window.__radarRouteStyleV188)return;window.__radarRouteStyleV188=true;
const MAIN='#5B21B6',ALT='#69B7FF',SRC='route-primary-v188',CASING='route-primary-v188-casing',LINE='route-primary-v188-line';
let lastSig='';
...
function install(){const a=app();if(!a?.map)return false;if(a.__routeStyleV188Installed)return true;a.__routeStyleV188Installed=true;
 ...
 setInterval(()=>{const x=app();if(x?.route)refresh();else if(x?.map?.getSource?.(SRC))clear();},900);
 if(a.route?.coords?.length)refresh();else clear();return true;}
let n=0,t=setInterval(()=>{n++;if(install()||n>250)clearInterval(t);},120);
window.RadarRouteStyleV127={paint:refresh,clear,main:MAIN,alternative:ALT,version:'188-thicker-stable-remaining'};
})();