/* Radar Seguro RJ PRO v188 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v188';
const CORE=[
  './radar-v188.html','./index.html','./manifest-v188.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v188.js',
  ...
  './route-style-v188.js','./route-cancel-cleanup-v188.js','./routing-integrity-v187.js','./navigation-recovery-v187.js','./route-alternatives-v187.js','./navigation-visual-v188.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v188.html'):Response.error());}}
