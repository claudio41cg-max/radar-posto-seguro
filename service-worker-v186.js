/* Radar Seguro RJ PRO v186 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v186';
const CORE=[
  './radar-v186.html','./index.html','./manifest-v186.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v186.js',
  ...
  './main-map-manual-hold-v168.js','./navigation-live-fix-v177.js','./navigation-guards-v178.js','./map-navigation-search-v182.js','./navigation-camera-v183.js','./route-style-v185.js','./route-cancel-cleanup-v186.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v186.html'):Response.error());}}
