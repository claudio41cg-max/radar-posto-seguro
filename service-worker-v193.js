/* Radar Seguro RJ PRO v193 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v193';
const CORE=[
  './radar-v193.html','./index.html','./manifest-v193.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v193.js','./navigation-arrow-v190.svg',
  ...
  './main-map-manual-hold-v168.js','./route-style-v188.js','./route-cancel-cleanup-v188.js','./navigation-visual-v190.js','./navigation-engine-v191.js',
  './search-professional-v193.js','./routing-professional-v193.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v193.html'):Response.error());}}
