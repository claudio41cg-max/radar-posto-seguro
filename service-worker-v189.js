/* Radar Seguro RJ PRO v189 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v189';
const CORE=[
  './radar-v189.html','./index.html','./manifest-v189.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v189.js','./navigation-arrow-v189.svg',
  ...
  './route-style-v188.js','./route-cancel-cleanup-v188.js','./routing-integrity-v187.js','./navigation-recovery-v187.js','./route-alternatives-v187.js','./navigation-visual-v189.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v189.html'):Response.error());}}
