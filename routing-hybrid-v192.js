...
let mode=loadMode(),oldAltRefresh=null;
function applyAlternativePolicy(){const api=window.RadarRouteAlternativesV116;if(!api)return;if(!oldAltRefresh&&typeof api.refresh==='function')oldAltRefresh=api.refresh.bind(api);if(mode==='radar'){try{api.clear?.();}catch(_){}if(oldAltRefresh)api.refresh=()=>{try{api.clear?.();}catch(_){}return null;};}else if(oldAltRefresh){api.refresh=oldAltRefresh;}}
function setMode(v,persist=true){v=String(v||'').toLowerCase();if(!['radar','tomtom','hybrid'].includes(v))return false;mode=v;if(persist)try{localStorage.setItem('radar.router.mode',v);}catch(_){}applyAlternativePolicy();const a=app();a?.toast?.(v==='radar'?'Radar Router ativado para teste.':v==='tomtom'?'TomTom ativada como roteador.':'Modo híbrido ativado.',2600);return true;}
async function ownRoute(a,start,end){if(!window.RadarOwnRouterV192?.route)throw new Error('Radar Router não carregado');try{window.RadarRouteAlternativesV116?.clear?.();}catch(_){}a.toast?.('Radar Router calculando pela malha OpenStreetMap...',2400);const r=await window.RadarOwnRouterV192.route(start,end,{transportMode:a.transportMode||'car'});try{a.prepareRouteGeometry?.(r);}catch(_){}return r;}
function install(){const a=app();if(!a?.map||!window.RadarOwnRouterV192?.route)return false;if(a.__routingHybridV192Installed)return true;const tomtomGet=typeof a.getRoute==='function'?a.getRoute.bind(a):null;if(!tomtomGet)return false;a.__routingHybridV192Installed=true;a.__tomTomGetRouteV192=tomtomGet;applyAlternativePolicy();
 ...
 const badge=()=>{try{const el=document.getElementById('gpsBadge');if(!el)return;el.title='Roteador: '+mode.toUpperCase();}catch(_){}};badge();setInterval(badge,4000);console.info('Radar v192 roteamento:',mode);return true;}
