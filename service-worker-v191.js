/* Radar Seguro RJ PRO v191 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v191';
const CORE=[
  './radar-v191.html','./index.html','./manifest-v191.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v191.js','./navigation-arrow-v190.svg',
  ...
  './search-access-v191.js','./routing-integrity-v191.js','./navigation-engine-v191.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v191.html'):Response.error());}}
