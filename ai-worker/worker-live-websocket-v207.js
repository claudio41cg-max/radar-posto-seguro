...
function allowedOrigin(request, env) {
  ...
  const allowed = String(
    env.ALLOWED_ORIGINS || DEFAULT_ORIGIN
  )
    .split(',')
...
async function normalizeUpstreamData(data) {
...
function handleLive(request, env) {
  if (!allowedOrigin(request, env)) {
    return new Response('Origem não autorizada.', {
      status: 403
    });
  }
  ...
  if (
    ...
    return new Response('WebSocket necessário.', {
      status: 426
    });
  }
  ...
  if (!apiKey) {
    return new Response('Chave Gemini não configurada.', {
      status: 503
    });
  }
  ...
  const upstream = new WebSocket(
    `${GEMINI_WS}?key=${encodeURIComponent(apiKey)}`
  );
  ...
  browser.addEventListener('message', event => {
    ...
    try {
      if (upstream.readyState === WebSocket.OPEN) {
        ...
        sendDiag(browser, 'client-message-forwarded');
      } else if (
        ...
        sendDiag(browser, 'gemini-websocket-not-open', {
  ...
  upstream.addEventListener('message', async event => {
    try {
      ...
      console.warn(
        'Radar Live v207 upstream relay',
        String(error?.message || error)
      );

      sendDiag(browser, 'upstream-normalize-error', {
  ...
  upstream.addEventListener('close', event => {
    ...
    sendDiag(browser, 'gemini-websocket-close', {
      code: event.code,
      reason: event.reason || ''
    });
