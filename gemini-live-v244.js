...
function renderLiveCloseDiag(){
  let el=document.getElementById('geminiLiveCloseDiagV245');
  if(!el){el=document.createElement('pre');el.id='geminiLiveCloseDiagV245';Object.assign(el.style,{position:'fixed',left:'18px',right:'18px',bottom:'18px',zIndex:'2147483647',margin:'0',padding:'12px',borderRadius:'14px',background:'rgba(0,0,0,.88)',color:'#fff',font:'700 14px/1.35 monospace',whiteSpace:'pre-wrap',pointerEvents:'none'});document.body.appendChild(el)}
  const d=liveCloseDiag;
  el.textContent=d?['DIAGNÓSTICO LIVE V245','LIVE: DESCONECTADO','TEMPO CONECTADO: '+(d.connectedForMs/1000).toFixed(1)+' s','FECHOU NA ETAPA: '+d.closedAt,'CÓDIGO WEBSOCKET: '+(d.code||'sem código'),'MOTIVO: '+(d.reason||'sem motivo'),'MANUAL: '+(d.manual?'SIM':'NÃO'),'MICROFONE ANTES DE FECHAR: '+(d.micActive?'ATIVO':'INATIVO'),'HORÁRIO: '+d.at].join('\n'):'';
}
