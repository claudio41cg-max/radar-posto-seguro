...
function stickyDiagV238(){
  try{
    ...
    el.textContent='DIAGNÓSTICO GEMINI V238\nMENSAGENS GEMINI: '+toolDiagV238.messages+'\nTOOL CALL: '+(toolDiagV238.toolCall?'SIM':'NÃO')+'\nFUNCTION CALL: '+(toolDiagV238.functionCall?'SIM':'NÃO')+'\nÚLTIMA FERRAMENTA: '+toolDiagV238.lastTool+'\nÁUDIO APÓS COMANDO: '+(toolDiagV238.audioAfterCommand?'SIM':'NÃO');
  }catch(e){}
}
...
async function executeRadarCommand(command){toolDiagV238.lastCommandAt=Date.now();stickyDiagV238();diag('EXECUTAR COMANDO',command);const text=String(command||'').trim();if(!text)return{ok:false,error:'Comando vazio'};const a=appRef();if(!a?.searchAddress)return{ok:false,error:'Busca do Radar indisponivel'};const m=text.match(/(?:me\s+leve|me\s+leva|leve(?:-|\s)?me|navegue|quero\s+ir|ir|vá|va|bora|vamos|vamo|trace(?:\s+uma)?\s+rota)(?:\s+(?:para|pra|pro|até|ate|em))?\s+(.+)/i);const destination=String(m?.[1]||text).replace(/[.!?]+$/,'').trim();if(!destination)return{ok:false,error:'Destino vazio'};try{diag('BUSCANDO DESTINO',destination);const results=await a.searchAddress(destination);diag('RESULTADO BUSCA',{quantidade:results?.length||0,primeiro:results?.[0]?.display||''});if(!results?.length)return{ok:false,error:'Destino nao encontrado',destination};const r=results[0];a.destination=[Number(r.lon),Number(r.lat)];const input=document.getElementById('destInput');if(input)input.value=r.display||destination;document.getElementById('suggest')?.classList.remove('show');if(!a.userPos){a.startGPS?.();const deadline=Date.now()+12000;while(!a.userPos&&Date.now()<deadline)await new Promise(x=>setTimeout(x,250));}if(!a.userPos)return{ok:false,error:'GPS ainda sem posicao',destination:r.display||destination};a.showRoutePanel?.();diag('CALCULANDO ROTA',r.display||destination);await a.calculateRoute?.();diag('RESULTADO ROTA',{temRota:Boolean(a.route),nav:Boolean(a.navActive)});if(!a.route)return{ok:false,error:'TomTom nao retornou rota',destination:r.display||destination};a.startNavigation?.();diag('NAVEGACAO INICIADA',{nav:Boolean(a.navActive)});return{ok:true,destination:r.display||destination,routeActive:true,navigationActive:Boolean(a.navActive)}}catch(e){return{ok:false,error:safeText(e?.message||e,180),destination}}}
async function handleToolCall(tc){diag('TOOL CALL RECEBIDO',tc);const calls=tc?.functionCalls||tc?.function_calls||[];if(!calls.length)return;const responses=[];for(const call of calls){const name=call?.name||'',id=call?.id||call?.callId;try{let result;if(name==='ler_contexto_radar')result={ok:true,context:radarContext()};else if(name==='executar_comando_radar')result=await executeRadarCommand(call?.args?.comando||call?.args?.command||'');else result=await window.RadarAiToolsV224?.execute?.(name,call?.args||{});responses.push({id,name,response:{result:result??null}})}catch(e){responses.push({id,name,response:{error:String(e?.message||e)}})}}if(responses.length){diag('FUNCTION RESPONSE',responses);send({toolResponse:{functionResponses:responses}})}}
