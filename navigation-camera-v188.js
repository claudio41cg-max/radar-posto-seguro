/* Radar Seguro RJ PRO v188 — mesma câmera da v183, com a seta ligeiramente mais baixa na tela. */
(()=>{
'use strict';
if(window.__radarNavigationCameraV188)return;
window.__radarNavigationCameraV188=true;
...
function install(){const a=app(),m=a?.map;if(!a||!m)return false;if(!a.__navigationLiveFixV177||!a.__navigationGuardsV178||!a.__mapNavigationSearchV182)return false;if(a.__navigationCameraV188)return true;a.__navigationCameraV188=true;S.nativeEase=m.easeTo?.bind(m)||null;
 ...
 if(a.navActive)setTimeout(()=>camera(a,true),250);return true;}
let tries=0,t=setInterval(()=>{tries++;if(install()||tries>300)clearInterval(t);},100);
window.RadarNavigationCameraV188={version:'188',refresh:()=>{const a=app();if(a){S.manualUntil=0;camera(a,true);}}};
})();