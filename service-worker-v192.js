/* Radar Seguro RJ PRO v192 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v192';
const CORE=[
  './radar-v192.html','./index.html','./manifest-v192.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v192.js','./navigation-arrow-v190.svg',
  ...
  './main-map-manual-hold-v168.js','./route-style-v188.js','./route-cancel-cleanup-v188.js','./route-alternatives-v187.js','./navigation-visual-v190.js',
  './search-access-v191.js','./routing-integrity-v191.js','./navigation-engine-v191.js','./radar-router-v192.js','./routing-hybrid-v192.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v192.html'):Response.error());}}
