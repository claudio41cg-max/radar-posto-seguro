...
const TEST_MARK='GEMINI-LIVE-v204';
...
function toast(text,ms=4200){try{const a=appRef();if(a?.toast)return a.toast(text,ms)}catch(e){}console.log('[Gemini Live v204]',text)}
function safeText(v,max=900){const s=String(v??'').replace(/\s+/g,' ').trim();return s.length>max?s.slice(0,max)+'…':s}

function ensureUI(){
  if(!document.getElementById('geminiLiveV204Style')){
    const s=document.createElement('style');
    s.id='geminiLiveV204Style';
    s.textContent=`#${MAIN_ID}.gemini-live-active{background:#0b5d3b!important;color:#effff6!important;box-shadow:0 0 0 3px rgba(71,255,153,.25),0 0 22px rgba(71,255,153,.55)!important;animation:livePulse204 1.25s ease-in-out infinite}#${NAV_ID}.gemini-live-active{background:#0b5d3b!important;color:#effff6!important;box-shadow:0 0 0 2px rgba(71,255,153,.22),0 0 18px rgba(71,255,153,.45)!important}#geminiLiveStatusV204{position:fixed;right:12px;top:118px;z-index:99999;padding:8px 11px;border-radius:999px;background:rgba(4,24,18,.95);border:1px solid rgba(103,255,178,.45);color:#ecfff4;font:700 11px/1.2 Arial,sans-serif;display:none;max-width:82vw;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;box-shadow:0 6px 22px rgba(0,0,0,.35)}#geminiLiveStatusV204.on{display:block}#geminiLiveDetailV204{position:fixed;left:12px;right:12px;bottom:88px;z-index:99999;padding:11px 12px;border-radius:12px;background:rgba(25,10,10,.97);border:1px solid rgba(255,120,120,.55);color:#fff;font:700 12px/1.4 Arial,sans-serif;display:none;box-shadow:0 8px 28px rgba(0,0,0,.42);white-space:pre-wrap;word-break:break-word;max-height:34vh;overflow:auto}#geminiLiveDetailV204.on{display:block}@keyframes livePulse204{0%,100%{transform:scale(1)}50%{transform:scale(1.045)}}`;
    document.head.appendChild(s);
  }
  if(!statusEl?.isConnected){statusEl=document.createElement('div');statusEl.id='geminiLiveStatusV204';statusEl.setAttribute('role','status');document.body.appendChild(statusEl)}
  if(!detailEl?.isConnected){detailEl=document.createElement('div');detailEl.id='geminiLiveDetailV204';document.body.appendChild(detailEl)}
}
...
function handleServerMessage(m){
  if(m?.setupComplete!==undefined){
    ...
    introTimer=setTimeout(()=>{
      if(running&&!micSending){micSending=true;setStatus('Gemini Live • ouvindo');showDetail('A sessão Live conectou. Pode falar com o Radar.',8000)}
    },6500);
    return;
  }
  ...
  for(const p of parts){
    ...
    playAudio(inline.data,rate).catch(e=>console.warn('Gemini Live v204 output',e));
  }
  if(m?.serverContent?.turnComplete){
    micSending=true;setStatus('Gemini Live • ouvindo');toast('Pode falar com o Radar.',1600);
  }
}

async function cleanup(closeSocket=false){
  clearTimeout(introTimer);introTimer=null;clearTimeout(setupTimer);setupTimer=null;clearTimeout(setupSendTimer);setupSendTimer=null;
  setupReady=false;running=false;starting=false;micSending=false;
...
async function start(){
  ...
  try{
    ...
    const join=LIVE_WS+(LIVE_WS.includes('?')?'&':'?')+'client='+encodeURIComponent(clientId)+'&v=204';
    ws=new WebSocket(join);
    ws.binaryType='arraybuffer';

    ws.onopen=()=>{
      stage='bridge-open';running=true;starting=false;setStatus('Gemini Live • ponte aberta…');
      setupSendTimer=setTimeout(()=>{
        if(!ws||ws.readyState!==WebSocket.OPEN)return;
        stage='gemini-setup';setStatus('Gemini Live • configurando 3.1…');
        ...
        setupTimer=setTimeout(()=>{
          ...
          setStatus('Gemini Live • aguardando confirmação…');
          showDetail('DIAGNÓSTICO GEMINI LIVE v204\nA ponte WebSocket abriu, mas o Gemini ainda não enviou setupComplete. A conexão continua aberta para diagnóstico.',30000);
        },9000);
      },900);
    };
    ws.onmessage=async e=>{
      ...
      }catch(err){console.warn('Gemini Live v204 message',err,e?.data)}
    };
    ws.onerror=e=>console.warn('Gemini Live v204 WebSocket',e);
    ws.onclose=async e=>{
      const manual=manualStop,code=Number(e?.code)||0,reason=String(e?.reason||'').trim(),closedAt=stage;
      console.warn('Gemini Live v204 closed',{code,reason,stage:closedAt,setupReady,introAudioReceived,micSending});
      await cleanup(false);
      if(!manual){
        const detail=`DIAGNÓSTICO GEMINI LIVE v204\nEtapa: ${closedAt}\nPonte WebSocket fechou • código ${code||'sem código'}${reason?' • '+reason:''}`;
        setStatus('Gemini Live • desconectado');showDetail(detail);toast('Gemini Live desconectou. Veja o diagnóstico na tela.',7000);
      }else setStatus('',false);
    };
  }catch(e){
    const failedAt=stage;console.warn('Gemini Live v204 start',{stage:failedAt,error:e});await cleanup(true);
    const denied=String(e?.name||'').toLowerCase().includes('notallowed');setStatus('',false);
    const msg=denied?'DIAGNÓSTICO GEMINI LIVE v204\nEtapa: microfone\nPermita o microfone no Chrome e tente novamente.':`DIAGNÓSTICO GEMINI LIVE v204\nEtapa: ${failedAt}\n${safeText(e?.message||e,900)}`;
    showDetail(msg);toast('Falha no Gemini Live v204. Veja o diagnóstico vermelho na tela.',7000);
  }
}
...
window.RadarGeminiLiveV204={start,stop,toggle,get state(){return{running,starting,setupReady,micSending,introAudioReceived,stage,model:MODEL,wsState:ws?.readyState??-1}}};
})();
