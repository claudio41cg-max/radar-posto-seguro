/* Radar Seguro RJ PRO v190 — Service Worker PWA. */
const CACHE_NAME='radar-seguro-pwa-v190';
const CORE=[
  './radar-v190.html','./index.html','./manifest-v190.json','./icon-192-1.png','./icon-512-1.png','./pwa-install-v190.js','./navigation-arrow-v190.svg',
  ...
  './main-map-manual-hold-v168.js','./navigation-live-fix-v177.js','./navigation-guards-v178.js','./map-navigation-search-v182.js','./navigation-camera-v188.js',
  './route-style-v188.js','./route-cancel-cleanup-v188.js','./routing-integrity-v187.js','./navigation-recovery-v187.js','./route-alternatives-v187.js','./navigation-visual-v190.js'
];
...
async function networkFirst(request){const cache=await caches.open(CACHE_NAME);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;}catch(_){return(await cache.match(request))||(request.mode==='navigate'?await cache.match('./radar-v190.html'):Response.error());}}
