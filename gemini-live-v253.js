...
let liveStartedAt=0,lastServerEvent='nenhum',lastServerEventAt=0,lastProxyEvent='nenhum',sawGoAway=false,serverMessageCount=0;
let audioPacketsSent=0,nonAudioSent=0,geminiPayloads=0,proxyPayloads=0,upstreamCloseCode='—',upstreamCloseReason='';
...
function send(obj){if(ws?.readyState===WebSocket.OPEN){if(obj?.realtimeInput?.audio)audioPacketsSent++;else nonAudioSent++;ws.send(JSON.stringify(obj));return true}return false}
...
function handleServerMessage(m){
  ...
  if(m?.goAway){sawGoAway=true;setStatus('Gemini Live • servidor vai reconectar');return} if(m?.toolCall){handleToolCall(m.toolCall).catch(e=>console.warn('Gemini Live tool',e));return}
...
async function start(){if(running||starting)return;stage='start';starting=true;manualStop=false;introAudioReceived=false;liveStartedAt=Date.now();lastServerEvent='nenhum';lastServerEventAt=0;lastProxyEvent='nenhum';sawGoAway=false;serverMessageCount=0;audioPacketsSent=0;nonAudioSent=0;geminiPayloads=0;proxyPayloads=0;upstreamCloseCode='—';upstreamCloseReason='';setButtons(true);showDetail('');setStatus('Gemini Live • preparando áudio…');try{try{voiceRef()?.clear?.()}catch(e){}try{assistantRef()?.stopHandsFree?.(false)}catch(e){}stage='audio';await Promise.all([prepareOutput(),prepareMic()]);stage='worker-websocket';setStatus('Gemini Live • abrindo ponte segura…');const join=LIVE_WS+'?client='+encodeURIComponent(clientId)+'&v=205';ws=new WebSocket(join);ws.onopen=()=>{stage='browser-websocket-open';running=true;starting=false;setStatus('Gemini Live • enviando configuração…');const toolDecl=[{name:'executar_comando_radar',description:'Executa no Radar Seguro um pedido de rota do motorista.',parameters:{type:'object',properties:{comando:{type:'string',description:'Frase objetiva do motorista, por exemplo: me leva para o Maracanã'}},required:['comando']}}];send({setup:{model:`models/${MODEL}`,generationConfig:{responseModalities:['AUDIO']},systemInstruction:{parts:[{text:SYSTEM_TEXT}]},tools:[{functionDeclarations:toolDecl}]}});setupTimer=setTimeout(()=>{if(running&&!setupReady){setStatus('Gemini Live • aguardando confirmação…');showDetail(`DIAGNÓSTICO GEMINI LIVE v205\nEtapa atual: ${stage}\nA ponte do celular está aberta, mas ainda não chegou setupComplete. Veja a etapa mostrada acima para saber até onde a mensagem avançou.`,0)}},7000)};ws.onmessage=async e=>{try{const text=await readMessageData(e.data);handleServerMessage(JSON.parse(text))}catch(err){console.warn('Gemini Live v205 message',err,e.data)}};ws.onerror=e=>console.warn('Gemini Live v205 WebSocket',e);ws.onclose=async e=>{const manual=manualStop,code=Number(e?.code)||0,reason=String(e?.reason||'').trim(),closedAt=stage;await cleanup(false);if(!manual){const duration=liveStartedAt?((Date.now()-liveStartedAt)/1000).toFixed(1):'0.0';const sinceLast=lastServerEventAt?((Date.now()-lastServerEventAt)/1000).toFixed(1):'—';const detail=`DIAGNÓSTICO GEMINI LIVE V253\nDuração: ${duration} s\nBrowser→Worker: ${code||'sem código'}${reason?' • '+reason:''}\nEtapa: ${closedAt}\nÚltimo Gemini: ${lastServerEvent}\nHá ${sinceLast} s\nWorker→Gemini close: ${upstreamCloseCode}${upstreamCloseReason?' • '+upstreamCloseReason:''}\nÚltimo proxy: ${lastProxyEvent}\nÁudio enviados: ${audioPacketsSent}\nOutros enviados: ${nonAudioSent}\nGemini recebidos: ${geminiPayloads}\nProxy recebidos: ${proxyPayloads}\nGOAWAY: ${sawGoAway?'SIM':'NÃO'}\nManual: ${manual?'SIM':'NÃO'}`;setStatus('Gemini Live • desconectado');showDetail(detail);toast('Gemini Live desconectou. Veja o diagnóstico.',7000)}else setStatus('',false)}}catch(e){const failedAt=stage;await cleanup(true);const denied=String(e?.name||'').toLowerCase().includes('notallowed');setStatus('',false);showDetail(denied?'DIAGNÓSTICO GEMINI LIVE v205\nPermita o microfone no Chrome e tente novamente.':`DIAGNÓSTICO GEMINI LIVE v205\nEtapa: ${failedAt}\n${safeText(e?.message||e,900)}`)}}
...
document.addEventListener('click',intercept,true);window.addEventListener('pagehide',()=>{manualStop=true;cleanup(true)});ensureUI();setButtons(false);window.RadarGeminiLiveV205={start,stop,toggle,get state(){return{running,starting,setupReady,micSending,introAudioReceived,stage,model:MODEL,wsState:ws?.readyState??-1}}};
})();